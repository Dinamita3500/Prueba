...
def preprocessing(img):
    ...
    img4_media = threshold_mean(img3)
    binary = img4_media>=img3
    image = img_as_float(binary)
    gimage = inverse_gaussian_gradient(image)
    init_ls = np.zeros(image.shape, dtype=np.int8)
    init_ls[10:-10, 10:-10] = 1
    ls = morphological_geodesic_active_contour(gimage, 250, init_ls,
                                           smoothing=1, balloon=-1,
                                           threshold=0.55,
                                           )
    img7=median(ls,disk(21))  

    minr=0
    minc=0
    maxr=0
    maxc=0
    cminr=np.int32(np.ones(20)*99)
    cminc=np.int32(np.ones(20)*99)
    cmaxr=np.int32(np.ones(20))
